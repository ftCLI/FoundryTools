...
class FontFinder:
    """
    Find fonts in a given path.

    It can search for fonts in a directory and its subdirectories, and can also handle a single font file.
    ...
    def __init__(
        ...
        try:
            ...
            msg = f"Invalid input path: {self.input_path}"
            raise FinderError(msg) from e
    ...
    def find_fonts(self) -> list[Font]:
        """
        Find fonts in the given input path.
    ...
    def find_collections(self) -> list[TTCollection]:
        """
        Find TTCollections in the given input path.
    ...
    def generate_fonts(self) -> Generator[Font, None, None]:
        """
        Generate a sequence of ``Font`` objects from the files generated by the ``_generate_files`` method.

        Filters out fonts based on specified conditions in ``_filter_conditions``.
        ...
        for file in files:
            try:
                ...
                if not any(condition and func(font) for condition, func in self._filter_conditions):
                    yield font
            except (TTLibError, PermissionError):  # noqa: PERF203
                pass

    def generate_collections(self) -> Generator[TTCollection, None, None]:
        """
        Generate a sequence of ``TTCollection`` objects from the files generated by the ``_generate_files`` method.
    ...
    def _validate_filter_conditions(self) -> None:
        if self.filter.filter_out_tt and self.filter.filter_out_ps:
            msg = "Cannot filter out both TrueType and PostScript fonts."
            raise FinderError(msg)
        if self.filter.filter_out_woff and self.filter.filter_out_woff2 and self.filter.filter_out_sfnt:
            msg = "Cannot filter out both web fonts and SFNT fonts."
            raise FinderError(msg)
        if self.filter.filter_out_static and self.filter.filter_out_variable:
            msg = "Cannot filter out both static and variable fonts."
            raise FinderError(msg)
